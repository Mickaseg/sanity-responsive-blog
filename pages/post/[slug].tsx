...
function Post({ post }: Props) {
  ...
  //Date refreshing with useEffect preventing render errors
  const [date, setDate] = useState(post._createdAt.toLocaleString());
  useEffect(() => {
    setDate(new Date(post._createdAt).toLocaleString());
  }, []);
        ...
        >
          ...
          <label className="block mb-5">
            ...
            <input
              ...
              className="shadow border rounded py-2 px-3 form-input mt-1 block w-full ring-yellow-500 outline-none focus:ring"
              type="email"
              placeholder="JohnAppelseed@gmail.com"
            />
          </label>
          <label className="block mb-5">
...
'comments': *[
  ...
  return {
    ...
    revalidate: 60, //after 60seconds, update old cash to current version
  };
};
